#Variational Mode Decomposition Based TimeDelay neural network Model
VMDTDNN=function(data,k,alpha,tau,K,DC,init,tol,l,n,r,m){
  ...
  for (i in 1:no_of_imf)
  {
    ...
    model_Ann<-forecast::nnetar(stats::as.ts(traindata), Lag= l,size=n, repeats=r, maxit=m)
    t=length(testdata)
    forecasted<- forecast::forecast(model_Ann,h=t)
  ...
  TotalIMF =  K
  output_f=list(Total_No_IMF=TotalIMF, Prediction_Accuracy_VMDTDNN =prediction_accuracy, Final_Prediction_VMDTDNN =final_prediction)
  return(output_f)
}
