...
VMDELM=function(data,k,alpha,tau,K,DC,init,tol){
  ...
  for (i in 1:no_of_imf)
  {
    ...
    traindata <- stats::as.ts(traindata)
    model_ELM<-nnfor::elm(traindata)
    t=length(testdata)
    forecasted<- forecast::forecast(model_ELM,h=t)
  ...
  TotalIMF =  K
  output_f=list(Total_No_IMF=TotalIMF, Prediction_Accuracy_VMDELM =prediction_accuracy, Final_Prediction_VMDELM =final_prediction)
  return(output_f)
}
